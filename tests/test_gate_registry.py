...
would feed that arithmetic a value nobody observed. So two checks, both of which
must fail on that day:

* a static scan: no arithmetic anywhere under `src/nfl_dfs/` takes a weather or
  roof value as an operand, except the few named non-numeric sites below, and
  none of those may sit in a scoring, projection or opportunity module;
* a behavioural check: every weather state, the derived-roof one included,
  gives byte-identical prior scores.
...
def weather_valued(node: ast.AST, tainted: set[str]) -> bool:
    ...
    if isinstance(node, ast.Subscript):
        ...
        if isinstance(key, ast.Constant) and isinstance(key.value, str) and WEATHER.search(key.value):
            return True
        return weather_valued(node.value, tainted)
    if isinstance(node, ast.Call):
        func = node.func
        if isinstance(func, ast.Attribute) and func.attr in PASS_THROUGH_METHODS:
            first = node.args[0] if node.args else None
            if (func.attr == "get" and isinstance(first, ast.Constant)
                    and isinstance(first.value, str) and WEATHER.search(first.value)):
                return True
