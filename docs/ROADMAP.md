...
Session 11b, subset binding, is in progress (claimed 2026-09-24). Session 12 is
the next startable row; Sessions 13 and 14 are startable too, but they edit
`selection.py`, `prior_review.py` and `cli.py`, which Session 11b edits, so do
not run them beside it. Session 12 shares only `docs/DATA_CONTRACTS.md` with it.
...
| 2026-09-24 | Session 11b | Pending to In Progress | claim commit | Claim pushed on `claude/affectionate-bohr-mnr8vz` |
