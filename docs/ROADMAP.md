...
Session 10 made the engine walk the rung ladder itself inside `run-slate`,
both modes, with every step recorded. Session 11, entry groups, is next;
Sessions 13 and 14 (which waited on 10) are startable too. Sessions 11 and 13
both edit `src/nfl_dfs/prior_review.py`, so run them one at a time; Session
14's Target Files overlap neither.
