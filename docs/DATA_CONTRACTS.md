...
keys are rejected. File paths may be absolute or relative to the request file.
...
The source-ledger hash is bound into the certification manifest; that binding
does not independently prove the source's truth, freshness, coverage, or rights.
...
evidence; they are not silently pulled from stale schedule rows.
...
`CONFLICTED`. Route participation is not present and remains `UNKNOWN` unless a
separately approved timely source is introduced.
...
changes only those blank roster cells; independent QA requires every other source
