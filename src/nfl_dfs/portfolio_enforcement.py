...
from .portfolio_policy import NormalizedPortfolioPolicy, canonical_decimal_json_bytes
...
def build_policy_candidate_bank(
    ...
    *,
    ...
    optimizer = LineupOptimizer(
        ...
        time_limit_seconds=min(total_budget, per_solve_budget),
    )
    ...
    status = "CANDIDATE_LIMIT_REACHED_INCOMPLETE"
    complete = False
    ...
    while len(candidates) < candidate_limit:
        ...
        if remaining <= 0:
            status = "CANDIDATE_BANK_TIME_LIMIT"
            terminal = "TOTAL_TIME_LIMIT"
            break
        optimizer.set_time_limit(min(per_solve_budget, remaining))
        cycle = solve_count
        perturbed = {
            row.dk_id: float(objective.get(row.dk_id, 0.0))
            + 1e-9 * (((cycle + 1) * (index + 17)) % 997)
            for index, row in enumerate(slate.players)
        }
        result = optimizer.solve(perturbed)
        solve_count += 1
        terminal = result.model_status
        if result.status == "INFEASIBLE":
            status = "COMPLETE_MODELED_BANK"
            complete = True
            break
        if result.status != "OPTIMAL" or result.roster is None:
            if result.model_status == "kTimeLimit":
                ...
                status = "CANDIDATE_BANK_SEARCH_LIMIT"
            else:
                status = "CANDIDATE_BANK_SOLVER_ERROR"
            break
        validation = validate_lineup(slate, result.roster)
        ...
        if canonical not in canonical_seen:
            candidates.append(
                PolicyCandidate(
                    ...
                    prior_points=sum(float(objective.get(dk_id, 0.0)) for dk_id in roster),
    ...
    return CandidateBank(
        ...
        total_time_limit_seconds=total_budget,
        per_solve_time_limit_seconds=per_solve_budget,
        elapsed_seconds=time.perf_counter() - started,
        solve_count=solve_count,
        terminal_model_status=terminal,
    )
