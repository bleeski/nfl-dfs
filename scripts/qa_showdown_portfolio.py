...
def main():
    ...
    for r in exp:
        ...
        if nq == 0:
            D.append(f"{r[0]} ZERO_QB")
        nk = sum(1 for m in meta if m['pos'] == 'K')
        if nk > 1:
            D.append(f"{r[0]} MULTIPLE_KICKERS {nk}")
        nd = sum(1 for m in meta if m['pos'] == 'DST')
        if nd > 1:
            D.append(f"{r[0]} MULTIPLE_DST {nd}")
        for m in meta:
            if m['pos'] == 'DST':
                off = [x for x in meta if x['team'] == m['team'] and x['pos'] != 'DST']
                if off:
                    D.append(f"{r[0]} DST_WITH_OWN_OFFENSE {m['team']}:{[x['name'] for x in off]}")
        for s, b in pairs:
