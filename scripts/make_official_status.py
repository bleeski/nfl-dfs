...
def _lock_from_game_info(rows) -> "datetime | None":
    """Read the kickoff out of `Game Info` so the window warning is data-driven."""
    ...
    if len(parts) < 3:
        return None
    stamp = " ".join(parts[-3:-1]) + " " + parts[-1]
    for fmt in ("%m/%d/%Y %I:%M%p ET", "%m/%d/%Y %I:%M%p"):
        try:
...
def main(argv: "list[str] | None" = None) -> int:
    ...
    if unknown:
        raise SystemExit(f"assignment IDs are not in the salary pool: {unknown}")
    stray = sorted(inactive.difference(selected))
    if stray:
        raise SystemExit(f"--inactive names IDs that are not selected: {stray}")
